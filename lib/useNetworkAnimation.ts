...
import { networkTheme, type NetworkThemeProfile, type NetworkVariant } from "./networkTheme";

type ParticleClass = "ambient" | "active" | "sync";
...
type GraphNode = {
  ...
  x: number;
  y: number;
  type: (typeof networkMap.nodes)[number]["type"];
};
...
type ScreenNode = GraphNode & {
  px: number;
  py: number;
  radius: number;
};

type ScreenEdge = GraphEdge & {
  ax: number;
  ay: number;
  bx: number;
  by: number;
...
type RuntimeState = {
  ...
  particles: Particle[];
  edgeEnergy: EnergyBuffers;
  nodeEnergy: EnergyBuffers;
  loading: boolean;
  graceUntil: number | null;
  visualOpacity: number;
  ambientSpawnBudget: number;
  routeSpawnBudget: number;
  syncSpawnBudget: number;
  particleCounter: number;
};

type UseNetworkAnimationOptions = {
  isLoading: boolean;
...
function createEdgeId(a: number, b: number) {
  return a < b ? `${a}${EDGE_KEY_SEPARATOR}${b}` : `${b}${EDGE_KEY_SEPARATOR}${a}`;
}

function lowerMiddleWeight(x: number, y: number) {
...
function createEnergyBuffers(length: number): EnergyBuffers {
  return {
...
function buildGraph() {
  ...
  const nodes: GraphNode[] = networkMap.nodes.map((node, index) => {
    nodeIndexById.set(node.id, index);
    return { ...node, index };
  });

  const edgeIndexById = new Map<string, number>();
  const edges: GraphEdge[] = networkMap.edges.map(([fromId, toId], index) => {
    const a = nodeIndexById.get(fromId);
    const b = nodeIndexById.get(toId);

    if (a === undefined || b === undefined) {
      throw new Error(`Unknown node id in edge: ${fromId} -> ${toId}`);
    }
  ...
  const routes: GraphRoute[] = networkMap.preferredRoutes.map((route) => {
    const routeIndices = route.map((nodeId) => {
      const index = nodeIndexById.get(nodeId);

      if (index === undefined) {
        throw new Error(`Unknown node id in route: ${nodeId}`);
      }
...
function pickWeightedIndex(weights: readonly number[]) {
...
function getParticleColor(kind: ParticleClass, palette: NetworkThemeProfile) {
  if (kind === "ambient") {
    return palette.pulseBlue;
  }

  if (kind === "active") {
    return palette.pulseOrange;
  }

  return palette.pulseWhite;
}

function getParticleChannel(kind: ParticleClass): keyof EnergyBuffers {
  if (kind === "ambient") {
    return "ambient";
  }
...
function getNodeRadius(type: GraphNode["type"], scale: number) {
  if (type === "hub") {
    return 3.8 * scale;
  }
...
function createGeometry(width: number, height: number): GeometryState {
  ...
  const edges = graph.edges.map((edge) => {
    ...
    const length = Math.max(Math.hypot(to.px - from.px, to.py - from.py), 1);
...
function choosePreferredRoute(kind: ParticleClass) {
...
function createParticle(runtime: RuntimeState, kind: ParticleClass, palette: NetworkThemeProfile): Particle {
  ...
  const speed = speedBase * scale * palette.speedMultiplier * (0.94 + Math.random() * 0.28);
  ...
  return {
    ...
    speed,
    radius:
      (kind === "ambient" ? 1.9 : kind === "active" ? 2.4 : 2.75) *
      scale *
...
function decayBuffer(buffer: Float32Array, dt: number, damping: number) {
...
function decayEnergies(runtime: RuntimeState, dt: number) {
  decayBuffer(runtime.edgeEnergy.ambient, dt, 3.2);
  decayBuffer(runtime.edgeEnergy.active, dt, 4.1);
  decayBuffer(runtime.edgeEnergy.sync, dt, 5.5);
...
function setEdgeEnergy(runtime: RuntimeState, from: number, to: number, kind: ParticleClass, value: number) {
  const edgeIndex = graph.edgeIndexById.get(createEdgeId(from, to));
  if (edgeIndex === undefined) {
    return;
  }

  const channel = getParticleChannel(kind);
  runtime.edgeEnergy[channel][edgeIndex] = Math.max(runtime.edgeEnergy[channel][edgeIndex], value);
}

function setNodeEnergy(runtime: RuntimeState, nodeIndex: number, kind: ParticleClass, value: number) {
  const channel = getParticleChannel(kind);
  runtime.nodeEnergy[channel][nodeIndex] = Math.max(runtime.nodeEnergy[channel][nodeIndex], value);
}

function updateTrail(particle: Particle, dt: number, maxPoints: number) {
...
function pushTrailPoint(particle: Particle, x: number, y: number) {
...
function updateParticles(runtime: RuntimeState, dt: number) {
  ...
  for (const particle of runtime.particles) {
    ...
    updateTrail(
      particle,
      dt,
      particle.kind === "ambient" ? AMBIENT_TRAIL_POINTS : ROUTE_TRAIL_POINTS,
    );
...
function drawBaseEdges(context: CanvasRenderingContext2D, runtime: RuntimeState, palette: NetworkThemeProfile) {
  context.save();
...
function drawEnergyEdges(
  context: CanvasRenderingContext2D,
  runtime: RuntimeState,
  channel: keyof EnergyBuffers,
  strokeStyle: string,
  lineWidth: number,
  shadowBlur: number,
) {
  ...
  for (let index = 0; index < runtime.geometry.edges.length; index += 1) {
    const strength = runtime.edgeEnergy[channel][index];
    if (strength <= 0.001) {
      continue;
    }

    const edge = runtime.geometry.edges[index];
    context.globalAlpha = clamp(strength, 0, 1);
    context.strokeStyle = strokeStyle;
    context.lineWidth = runtime.geometry.scale * (lineWidth + strength * 1.8);
    context.shadowColor = strokeStyle;
    context.shadowBlur = shadowBlur * runtime.geometry.scale * strength;
    context.beginPath();
    context.moveTo(edge.ax, edge.ay);
    context.lineTo(edge.bx, edge.by);
    context.stroke();
...
function drawNodes(context: CanvasRenderingContext2D, runtime: RuntimeState, palette: NetworkThemeProfile) {
  context.save();
...
function drawParticles(context: CanvasRenderingContext2D, runtime: RuntimeState, palette: NetworkThemeProfile) {
  context.save();
...
function shouldSpawnAmbient(runtime: RuntimeState) {
  return runtime.particles.length < 54;
}

function shouldSpawnRoute(runtime: RuntimeState, variant: NetworkVariant) {
  return runtime.particles.length < (variant === "light" ? 38 : 48);
}

function updateSpawn(runtime: RuntimeState, palette: NetworkThemeProfile, variant: NetworkVariant, dt: number) {
  if (!runtime.loading) {
    return;
  }
...
function renderFrame(context: CanvasRenderingContext2D, runtime: RuntimeState, palette: NetworkThemeProfile) {
...
function resizeCanvas(canvas: HTMLCanvasElement, runtime: RuntimeState) {
...
export function useNetworkAnimation({ isLoading, variant }: UseNetworkAnimationOptions) {
  ...
  useEffect(() => {
    ...
    const runtime: RuntimeState = {
      ...
      particles: [],
      edgeEnergy: createEnergyBuffers(graph.edges.length),
    ...
    const step = (now: number) => {
      ...
      const previousTimestamp = (step as typeof step & { previous?: number }).previous ?? now;
      (step as typeof step & { previous?: number }).previous = now;

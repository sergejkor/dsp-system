...
export default function SettingsAdvancedPage() {
  ...
  function handleSave() {
    ...
    setSaving(true);
    updateSettingsGroup('advanced', payload).then((updated) => { setData(updated); setDraft(updated); setMessage('Saved.'); }).catch((e) => setError(e.message)).finally(() => setSaving(false));
  }

  function handleReset() {
    if (!confirm('Restore default values for Advanced settings?')) return;
    setSaving(true);
    resetSettingsGroup('advanced').then((updated) => { setData(updated); setDraft(updated); setMessage('Defaults restored.'); }).catch((e) => setError(e.message)).finally(() => setSaving(false));
  }

  if (loading) return <p className="muted">Loading…</p>;
  if (error) return <p className="settings-msg settings-msg--err">{error}</p>;
  if (items.length === 0) return <p className="muted">No advanced settings defined. Add items in the advanced group via seed or API.</p>;

  return (
    <>
      <h3>Advanced / System Config</h3>
      <p className="muted">Timezone, language, pagination, maintenance mode, system banner.</p>
      ...
      <style>{`.settings-form { display: flex; flex-direction: column; gap: 0.75rem; max-width: 520px; } .settings-row { display: flex; align-items: center; gap: 0.5rem; } .settings-label { min-width: 200px; } .settings-actions { margin-top: 1rem; display: flex; gap: 0.5rem; } .settings-msg { padding: 0.5rem; border-radius: 6px; } .settings-msg--ok { background: #d1fae5; color: #065f46; } .settings-msg--err { background: #fee2e2; color: #991b1b; }`}</style>
    </>
  );
}

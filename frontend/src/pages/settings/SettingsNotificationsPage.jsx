...
export default function SettingsNotificationsPage() {
  ...
  useEffect(() => {
    getSettingsByGroup('notifications')
  ...
  function handleSave() {
    ...
    setSaving(true);
    updateSettingsGroup('notifications', payload).then((updated) => { setData(updated); setDraft(updated); setMessage('Saved.'); }).catch((e) => setError(e.message)).finally(() => setSaving(false));
  }

  if (loading) return <p className="muted">Loading…</p>;
  if (error) return <p className="settings-msg settings-msg--err">{error}</p>;
  if (items.length === 0) return <p className="muted">No notification settings defined.</p>;

  return (
    <>
      <h3>Notifications</h3>
      <p className="muted">Email, SMS, reminders, and alert preferences.</p>

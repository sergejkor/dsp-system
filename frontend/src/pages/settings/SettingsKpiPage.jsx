...
export default function SettingsKpiPage() {
  ...
  function handleSave() {
    ...
    setSaving(true);
    setError('');
    updateSettingsGroup('kpi', payload)
      .then((updated) => {
        setData(updated);
        setDraft(updated);
        setMessage('KPI settings saved.');
      })
      .catch((e) => setError(e.message))
  ...
  function handleReset() {
    if (!confirm('Restore default KPI values?')) return;
    setSaving(true);
    resetSettingsGroup('kpi')
      .then((updated) => {
        setData(updated);
        setDraft(updated);
  ...
  if (items.length === 0) return <p className="muted">No KPI settings. Run seed:settings to create defaults.</p>;

  return (
    <>
      <h3>KPI Settings</h3>
      <p className="muted">Thresholds and targets used across the app. Edit without code changes.</p>
      ...
      <style>{`
        ...
        .settings-label { min-width: 200px; font-size: 0.9rem; }

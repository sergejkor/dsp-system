...
export default function SettingsRoutesPage() {
  ...
  function handleSave() {
    ...
    setSaving(true);
    updateSettingsGroup('routes', payload).then((updated) => { setData(updated); setDraft(updated); setMessage('Saved.'); }).catch((e) => setError(e.message)).finally(() => setSaving(false));
  }

  if (loading) return <p className="muted">Loading…</p>;
  if (error) return <p className="settings-msg settings-msg--err">{error}</p>;
  if (items.length === 0) return <p className="muted">No route/dispatch settings defined.</p>;

  return (
    <>
      <h3>Route / Dispatch Settings</h3>
      <p className="muted">Route statuses, completion thresholds, rescue labels.</p>

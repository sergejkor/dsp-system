...
export default function EVMonitoringPage() {
  ...
  const cards = [['Total EV', result.summary.totalVehicles], ['At least 90%', result.summary.okVehicles], ['Below 90%', result.summary.belowThreshold], ['Data issues', result.summary.errorVehicles + result.summary.staleVehicles]];
  return <main className="ev-page"><header className="ev-header"><div><span>FLEET / EV</span><h1>EV Monitoring</h1><p>Last live check: {dateTime}</p></div></header>{error && <p className="ev-error">LIVE refresh failed — displaying the most recent stored check.</p>}
    ...
    <section className="ev-table-wrap"><table className="ev-table"><thead><tr><th>Vehicle</th><th>Source</th><th>SOC</th><th>Charging</th><th>Battery Power</th><th>Telemetry</th><th>Status</th></tr></thead><tbody>{result.vehicles.map((vehicle) => <tr key={`${vehicle.source}:${vehicle.externalId}`}><td>{vehicle.vehicleName}</td><td>{vehicle.source === 'rivian' ? 'Rivian' : 'Geotab'}</td><td><strong className={vehicle.soc !== null && vehicle.soc < result.threshold ? 'low' : 'good'}>{vehicle.soc === null ? '—' : `${vehicle.soc}%`}</strong></td><td>{dash(vehicle.chargingState)}</td><td>{vehicle.chargingPowerW === null ? '—' : `${vehicle.chargingPowerW} W`}</td><td>{vehicle.socTimestamp ? new Intl.DateTimeFormat(locale, { dateStyle: 'short', timeStyle: 'short' }).format(new Date(vehicle.socTimestamp)) : '—'}{vehicle.stale ? ' · stale' : ''}</td><td><span className={`ev-status ${vehicle.status}`}>{vehicle.status}</span></td></tr>)}</tbody></table>{!result.vehicles.length && <p className="ev-empty">No EV telemetry is currently available.</p>}</section>
  </main>;
}

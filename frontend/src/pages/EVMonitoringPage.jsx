...
const socClass = (value) => value === null || value === undefined ? 'soc-unknown' : Number(value) < 20 ? 'soc-critical' : Number(value) < 80 ? 'soc-warning' : 'soc-good';
function chargingStateText(value) { if (value === null || value === undefined || value === '') return '—'; if (typeof value === 'string' || typeof value === 'number') return String(value); if (typeof value === 'object') return value.formattedValue || value.value || value.status || '—'; return '—'; }
...
export default function EVMonitoringPage() {
  ...
  if (!result) return <main className="ev-page"><div className="ev-loading" role="status"><span className="ev-spinner" aria-hidden="true" /><span>EV-Daten werden aktualisiert...</span></div>{error && <p className="ev-error">{error}</p>}</main>;
  ...
  return <main className="ev-page"><header className="ev-header"><div><span>FLEET / EV</span><h1>EV Monitoring</h1><p>Last live check: {dateTime}</p></div></header>{error && <p className="ev-error">LIVE refresh failed — displaying the most recent stored check.</p>}
    ...
    <section className="ev-table-wrap"><table className="ev-table"><thead><tr><th>Vehicle</th><th>Source</th><th>SOC</th><th>Charging</th><th>Telemetry</th><th>Status</th></tr></thead><tbody>{result.vehicles.map((vehicle) => <tr key={`${vehicle.source}:${vehicle.externalId}`}><td>{vehicle.vehicleName}</td><td>{vehicle.source === 'rivian' ? 'Rivian' : 'Mercedes eSprinter 350'}</td><td><strong className={socClass(vehicle.soc)}>{formatSoc(vehicle.soc)}</strong></td><td>{chargingStateText(vehicle.chargingState)}</td><td>{vehicle.socTimestamp ? new Intl.DateTimeFormat(locale, { dateStyle: 'short', timeStyle: 'short' }).format(new Date(vehicle.socTimestamp)) : '—'}{vehicle.stale ? ' · stale' : ''}</td><td><span className={`ev-status ${vehicle.status}`}>{vehicle.status}</span></td></tr>)}</tbody></table>{!result.vehicles.length && <p className="ev-empty">No EV telemetry is currently available.</p>}</section>
  </main>;
}

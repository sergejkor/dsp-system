...
test('buildEvResult classifies threshold, stale and missing SOC and orders priority', () => {
  ...
  assert.deepEqual(result.vehicles.map((v) => v.vehicleName), ['Missing', 'Low', 'Healthy']); assert.equal(result.summary.belowThreshold, 1); assert.equal(result.summary.errorVehicles, 1);
});

      ...
      return await withContext('rivian', env.EV_MONITORING_RIVIAN_PROFILE_DIR, async (context) => {
        ...
        while (offset < total) {
          const response = await page.evaluate(async ({ apiUrl, offset, limit, assetGroup }) => {
            const res = await fetch(apiUrl, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ operationName: 'Vehicles', variables: { assetGroups: [assetGroup], filter: '{}', limit, offset, sort: [{ attr: 'core/info/identifiers.vin', order: 'DESC' }] }, query: 'query Vehicles($assetGroups:[String!]!,$limit:Int,$offset:Int,$sort:[FleetVehiclesPaginatedSortInput!],$filter:String) { fleetVehiclesPaginated(assetGroups:$assetGroups, filter:$filter, limit:$limit, offset:$offset, sort:$sort) { items { id name vin telemetry { lastReportedAt battery { main { stateOfCharge { value timestamp } chargerState chargingPower } } } } pageInfo { totalItems } } }' }) });
